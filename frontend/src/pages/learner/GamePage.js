...
const games = [
  ...
  },
  {
    id: 'coming3',
    title: 'Language Lab',
    subtitle: 'Linguistic Adventures',
    description: 'Learn new languages through immersive word games and cultural challenges.',
    icon: '🌍',
    color: '#2de08e',
    accent: '#50f0a0',
    tag: 'Coming Soon',
    ...
    xp: 400,
    difficulty: 'Beginner',
    topics: ['Vocabulary', 'Grammar', 'Pronunciation'],
    available: false,
  },
];
    ...
    <div style={{ minHeight: '100vh', background: '#07060f', color: 'white' }}>
      ...
      <div style={{ padding: '16px 32px', background: 'rgba(7,5,15,0.95)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 16, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 100 }}>
        <button onClick={() => navigate('/dashboard')}
          style={{ padding: '8px 18px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 13 }}>
          ← Dashboard
        </button>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 32px' }}>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'rgba(123,94,167,0.15)', border: '1px solid rgba(123,94,167,0.3)', borderRadius: 99, marginBottom: 20, fontSize: 12, fontWeight: 700, color: '#9d7fd4', letterSpacing: 1, textTransform: 'uppercase' }}>
            🎮 Learning Games
          </div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 18, maxWidth: 560 }}>
            Gamified learning experiences designed to make complex topics fun. Complete missions, earn XP, and climb the leaderboard!
          </p>
        </div>
        ...
        <div style={{ padding: '20px 28px', background: 'linear-gradient(135deg, rgba(123,94,167,0.15), rgba(232,84,122,0.1))', border: '1px solid rgba(123,94,167,0.25)', borderRadius: 20, marginBottom: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          ...
          <button onClick={() => navigate('/dashboard')}
            style={{ padding: '10px 24px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: 'rgba(255,255,255,0.7)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, fontSize: 13 }}>
            View Leaderboard →
          </button>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.3)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 24 }}>
            ▶ Available Now
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 20, marginBottom: 48 }}>
            {games.filter(g => g.available).map(game => (
              <div key={game.id}
                ...
                style={{ background: 'rgba(14,11,26,0.8)', border: `1px solid ${hoveredGame === game.id ? game.color + '60' : 'rgba(255,255,255,0.08)'}`, borderRadius: 24, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.3s', transform: hoveredGame === game.id ? 'translateY(-6px)' : 'translateY(0)', boxShadow: hoveredGame === game.id ? `0 20px 60px ${game.color}25` : 'none' }}>

                {/* Game Banner */}
                <div style={{ height: 160, background: `linear-gradient(135deg, ${game.color}40, ${game.accent}20)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'absolute', top: 14, right: 14, padding: '4px 12px', background: 'rgba(45,224,142,0.2)', border: '1px solid rgba(45,224,142,0.4)', borderRadius: 99, fontSize: 11, fontWeight: 700, color: '#2de08e' }}>
                    AVAILABLE
                  </div>
                  <div style={{ position: 'absolute', top: 14, left: 14, padding: '4px 12px', background: `${game.color}30`, border: `1px solid ${game.color}50`, borderRadius: 99, fontSize: 11, fontWeight: 700, color: game.accent }}>
                    {game.tag}
                  </div>
                </div>
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
                    {game.topics.map(topic => (
                      <span key={topic} style={{ padding: '3px 10px', background: `${game.color}15`, border: `1px solid ${game.color}30`, borderRadius: 99, fontSize: 11, fontWeight: 600, color: game.accent }}>
                        {topic}
                      </span>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 20, marginBottom: 20, padding: '12px 0', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#ffd700' }}>+{game.xp}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>Max XP</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: 'white' }}>{game.levels}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>Levels</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 15, fontWeight: 800, color: difficultyColor[game.difficulty] }}>{game.difficulty}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>Difficulty</div>
                    </div>
                  </div>

                  <button style={{ width: '100%', padding: '14px', background: `linear-gradient(135deg, ${game.color}, ${game.accent})`, border: 'none', borderRadius: 14, color: 'white', cursor: 'pointer', fontSize: 15, fontWeight: 800, fontFamily: 'inherit', boxShadow: `0 4px 20px ${game.color}40` }}>
                    🎮 Play Now — Earn XP
                  </button>
                </div>
              </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {games.filter(g => !g.available).map(game => (
              <div key={game.id}
                style={{ background: 'rgba(14,11,26,0.5)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 20, overflow: 'hidden', opacity: 0.7 }}>
